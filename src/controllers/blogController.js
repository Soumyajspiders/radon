...
//             if (authorCount <= 0) {
//                 res.status(400).send({ msg: "error-response-structure " });
//             }
//         } else {
...
//         res.status(201).send(blogPop, { msg: 'successful-response-structure' });
//     } catch (err) {
...
//         let data = req.body;
//         let checkData = await blogModel.findOne({ _Id: id, isDeleted: false });
//         if (!checkData) {
...
//                 publisedAt: Date.now()
//             },
//             $push: { subcatagory: data.subcatagory, tags: { $each: data.tags } },
//         };
...
//     await blogsModel.findOneAndUpdate(query, {$set: { isDeleted: true, deletedAt: Date.now() }});
  
//     res.status(200).send();
//   };

//   const getBlogs= async (seq,res)=>{
//     try{
//         let authorId=req.quary.authorId
//         let tags=req.quary.tags
//         let subcatagory=req.quary.subcatagory
...
//         }
//         if (subcatagory){
//             obj.subcatagory=subcatagory
//         }

//         let savedata =await blogModel.find(obj)
//         if(savedata.length == 0){
//             return res.status(400).send({status:false,msg:"no such blog available"})
//         }else{
//             return res.status(200).send({msg:savedata})
//         }
//     }catch(err){
